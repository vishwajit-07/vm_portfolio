...
export default function AdminExperiences() {
  ...
  const syncExperience = async (updatedExperiences: any[]) => {
    setSaving(true); setError('');
    try {
      // Cleanup: Remove temporary _ids from new items
      const cleaned = updatedExperiences.map(item => {
        const { _id, ...rest } = item;
      ...
      setError(e.response?.data?.message || 'Failed to synchronize experience data.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    let updated;
    if (editing) {
      updated = items.map((item: any) => item._id === editing._id ? { ...item, ...newExp } : item);
    } else {
      updated = [...items, { ...newExp, _id: Date.now().toString() }]; // Temp ID, backend generates real one
    }
    await syncExperience(updated);
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Permanently delete this work experience?')) return;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-5xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tighter gradient-text-orange uppercase font-display">Career Timeline</h1>
          <p className="text-[12px] text-muted-foreground mt-1 font-medium">Manage your professional milestones and organizational history.</p>
        </div>
          ...
          >
            ...
            >
              ...
              <h2 className="text-xl md:text-2xl font-black mb-6 gradient-text-orange uppercase tracking-tighter font-display">
                {editing ? 'Refine Experience' : 'New Milestone'}
              </h2>
              ...
              <form onSubmit={handleSave} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                   <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black tracking-widest text-muted-foreground ml-1">Job Title / Role</label>
                    <Input required value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} className="h-11 bg-background/40 border-orange-500/10 rounded-xl text-sm" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black tracking-widest text-muted-foreground ml-1">Organization Name</label>
                    <Input required value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} className="h-11 bg-background/40 border-orange-500/10 rounded-xl text-sm" />
                  </div>
                </div>
                
                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase font-black tracking-widest text-muted-foreground ml-1">Temporal Duration</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase font-black tracking-widest text-muted-foreground ml-1">Impact Description</label>
                  <textarea
                ...
                <div className="flex gap-3 pt-2">
                  <Button type="submit" disabled={saving} className="flex-1 h-12 rounded-xl font-black uppercase tracking-widest text-[10px]">
                    {saving ? 'Synchronizing...' : editing ? 'Update Chronicle' : 'Forge Milestone'}
                  </Button>
