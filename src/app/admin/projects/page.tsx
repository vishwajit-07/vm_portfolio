...
const EMPTY = { title: '', description: '', techStack: '', githubLink: '', liveLink: '', featured: false };
  ...
  return (
    ...
    >
      <div className="relative h-full w-full bg-[#0B0B0F]/40 backdrop-blur-3xl rounded-[1.4rem] p-8 flex flex-col overflow-hidden">
        {item.featured && (
          <div className="absolute top-6 right-6 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 text-[8px] font-black uppercase tracking-[0.2em] shadow-[0_0_15px_rgba(249,115,22,0.2)]">
            Featured
          </div>
        )}
        ...
        <div className="pt-6 border-t border-white/[0.05] mt-auto flex flex-wrap gap-1.5">
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-2xl glass-card rounded-[2rem] border border-white/[0.08] p-8 md:p-10 shadow-2xl relative bg-[#0B0B0F]/40"
            >
              <div className="absolute top-0 right-0 w-48 h-48 bg-orange-500/5 blur-[80px] rounded-full -mr-24 -mt-24 pointer-events-none" />
              ...
              <form onSubmit={handleSave} className="space-y-6">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                        <label className="text-[9px] uppercase font-black tracking-widest text-gray-500 ml-1">Identifier</label>
                        <Input required value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="h-11 bg-white/[0.02] border-white/[0.05] rounded-xl px-4 focus:border-orange-500/30 transition-all outline-none text-sm" placeholder="Project Name" />
                    </div>
                     <div className="space-y-1.5">
                        <label className="text-[9px] uppercase font-black tracking-widest text-gray-500 ml-1">Priority</label>
                        <select 
                          value={form.featured ? 'true' : 'false'} 
                          onChange={e => setForm({...form, featured: e.target.value === 'true'})}
                          className="w-full h-11 bg-white/[0.02] border border-white/[0.05] rounded-xl px-4 text-[13px] font-bold focus:border-orange-500/30 outline-none transition-all appearance-none cursor-pointer text-gray-300"
                        >
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase font-black tracking-widest text-gray-500 ml-1">Strategic Narrative</label>
                  <textarea required value={form.description} onChange={e => setForm({...form, description: e.target.value})} rows={2} className="w-full rounded-xl border border-white/[0.05] bg-white/[0.02] px-4 py-3 text-[13px] font-medium focus:border-orange-500/30 outline-none transition-all resize-none leading-relaxed text-gray-300" placeholder="Describe the architectural complexity..." />
                </div>

                <div className="space-y-1.5">
                  <label className="text-[9px] uppercase font-black tracking-widest text-gray-500 ml-1">Technical Component Stack</label>
                  <Input required value={form.techStack} onChange={e => setForm({...form, techStack: e.target.value})} placeholder="Next.js, TypeScript, MERN..." className="h-11 bg-white/[0.02] border-white/[0.05] rounded-xl px-4 focus:border-orange-500/30 transition-all outline-none text-sm" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black tracking-widest text-gray-500 ml-1">Git Repository</label>
                    <Input value={form.githubLink} onChange={e => setForm({...form, githubLink: e.target.value})} placeholder="github.com/..." className="h-11 bg-white/[0.02] border-white/[0.05] rounded-xl px-4 focus:border-orange-500/30 transition-all outline-none text-sm" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[9px] uppercase font-black tracking-widest text-gray-500 ml-1">Deployment</label>
                    <Input value={form.liveLink} onChange={e => setForm({...form, liveLink: e.target.value})} placeholder="https://..." className="h-11 bg-white/[0.02] border-white/[0.05] rounded-xl px-4 focus:border-orange-500/30 transition-all outline-none text-sm" />
                  </div>
                </div>

                <div className="flex gap-3 pt-4">
                  <Button type="submit" disabled={saving} className="flex-1 h-12 rounded-xl font-black uppercase tracking-widest text-[10px] bg-orange-500 text-white shadow-premium transition-all active:scale-95">
                    {saving ? 'Synchronizing...' : 'Commit Change →'}
                  </Button>
                  <Button type="button" variant="outline" onClick={() => setShowModal(false)} className="w-28 h-12 rounded-xl border-white/5 hover:bg-white/5 font-black uppercase tracking-widest text-[9px] text-gray-500 hover:text-white transition-all">
                    Abort
                  </Button>
                </div>
              </form>
